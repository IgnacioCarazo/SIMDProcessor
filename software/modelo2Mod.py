...
class AesFromScratch:
    ...
    def initSteMat(self, plaintext):
        ...
        while plaintext:
            state.append(plaintext[:2])#inserta 2 digitos "hex" 
            plaintext = plaintext[2:]# corre el strin 2

        return state

    # Opertaion Functions
    def xor(self, left, right):
        out = hex(int(left, 16) ^ int(right, 16))[2:]
        return out.zfill(2)
    ...
    def keyAddition(self, state, key):
        ...
        stateString = self.xor(stateString, keyString).zfill(len(stateString))#xor
    ...
    def byteSubstitution(self, state):
        ...
        for byte in state:
            out = int("0x{}".format(byte), 0)#convierte de "hex" a numero para buscar en la tabla sbox
            out = str(hex(self.sBox[out]))[2:]
            ...
            newState.append(out)
        #print(newState)
        return newState #retorna una lista 
    ...
    def encrypt10round(self, plaintext, key):
        ...
        state = self.initSteMat(plaintext)# esto separa cada 2 caracteres

        my_matrix = np.array(state).reshape((4, 4)).T #hace una matriz 4*4
        rklist = textwrap.wrap(key, 2)# esto separa cada 2 caracteres
        my_matrix = np.array(rklist).reshape((4, 4)).T #hace una matriz 4*4
        ...
        for round in range(10):
            state = self.byteSubstitution(state)#cambio de bits
            my_matrix = np.array(state).reshape((4, 4)).T #hace una matriz 4*4
            state = self.shiftRows(state) # cambio de filas
            my_matrix = np.array(state).reshape((4, 4)).T #hace una matriz 4*4
            if round < 9:
